...
public class ReadVehicleXml {
	...
	public List<Vehicle> readConfig(String configFile) {
		List<Vehicle> vehiclesList = new ArrayList<Com.Vehicle.Vehicle>();
		Car car = null;
		Bike bike = null;
			...
			while (eventReader.hasNext()) {
				...
				if (event.isStartElement()) {
					 ...
					 */
						...
						event = eventReader.nextEvent();
						if (vehicle instanceof Car) {
							car.calculateOnRoadPrice(Integer.parseInt(event
									.asCharacters().getData()));
						} else if (vehicle instanceof Bike) {
							bike.calculateOnRoadPrice(Integer.parseInt(event
									.asCharacters().getData()));
						}
					} else if (startElement
							.getName()
