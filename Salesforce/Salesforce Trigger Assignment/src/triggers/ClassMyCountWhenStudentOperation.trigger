trigger ClassMyCountWhenStudentOperation on Student__c (before insert,before update,before delete,after undelete) {
   if(Trigger.isUpdate)
   {
       for(Student__c stdNew : Trigger.new)
       {
           ...
           {
              Class__c clsNew = [SELECT Id,My_Count__c FROM Class__c WHERE Id =:stdNew.Class__c ] ;
              clsNew.My_Count__c = clsNew.My_Count__c +1;
              update clsNew;
              Class__c clsOld = [SELECT Id,My_Count__c FROM Class__c WHERE Id =:stdOld.Class__c ] ;
              clsOld.My_Count__c = clsOld.My_Count__c - 1;
               update clsOld;
           }
       }
   }
