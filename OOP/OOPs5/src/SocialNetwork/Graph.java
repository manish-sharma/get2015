...
public class Graph {
	...
	public int addNode(Node node)
	{
		try
		{
			...
			{
				if(iteratorNodeSet.next().getEmailId().equals(entity.getEmailId()))//Email id is unique key
				{
					return -1;
				}
			}
	...
	public int addEdge(String name1, String name2)
	{
		try
		{
			...
			{
				Iterator<Entity> itr = entityForFristPerson.setFriends.iterator();
				while(itr.hasNext())
				{
					if(itr.next().getEmailId().equals(entityForSecondPerson.getEmailId()))
					{
						checkForFriend = true;
					}
				}
