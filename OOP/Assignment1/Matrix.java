...
public class Matrix {
	 ...
	 * @return It return a length of an array
	 */
	public int[][] UserInputInArray(int row, int col) {
		...
		try {
			for (int i = 0; i < row; i++)
				for (int Col = 0; Col < col; Col++)
					input[i][Col] = UserInput();
			return input;

		} catch (InputMismatchException e) {
			System.out.println("enter the correct input");
			input = UserInputInArray(row, col);
			return input;
		}
	...
	/** shows the particular matrix of object which calls it.
	 * 
	 */
	public void show() {
