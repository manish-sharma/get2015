...
public class Graph {
	...
	public void addEdge(Entity entityFirst, Entity entitySecond) {
		Iterator<Entity> itr1 = Processing.nodeSet.iterator(); // Iterating list
																// of
																// connections
		while (itr1.hasNext()) {
			if (itr1.next().getEmail().equals(entitySecond.getEmail())) {
				if (Processing.friendMap.containsKey(entityFirst.getEmail())) {
					if (!Processing.friendMap.get(entityFirst.getEmail())
							.contains(entitySecond.getEmail())) {
						Processing.friendMap.get(entityFirst.getEmail()).add(
								entitySecond.getEmail());
						System.out.println(entitySecond.getEmail()
								+ " is now friend of you");
					} else if (Processing.friendMap.get(entityFirst.getEmail())
							.contains(entitySecond.getEmail())) {
						System.out.println(entitySecond.getEmail()
								+ " is already friend of you");
					} else {
						System.out.println(entitySecond.getEmail()
								+ " does not exist");
					}
				} else {
					Iterator<Entity> itr2 = Processing.nodeSet.iterator(); // Iterating
																			// list
																			// of
																			// entities
					while (itr2.hasNext()) {
						if (itr2.next().getEmail()
								.equals(entityFirst.getEmail())) {
							Set<String> set = new HashSet<String>();
							set.add(entitySecond.getEmail());
							Processing.friendMap.put(entityFirst.getEmail(),
									set);
							System.out.println(entitySecond.getEmail()
									+ " is now friend of you");
						}
	...
	public void removeEdge(Entity entityFirst, Entity entitySecond) {
		if (Processing.friendMap.containsKey(entityFirst.getEmail())) {
			if (Processing.friendMap.get(entityFirst.getEmail()).contains(
					entitySecond.getEmail())) {
				Processing.friendMap.get(entityFirst.getEmail()).remove(
						entitySecond.getEmail());
				System.out.println(entitySecond.getEmail()
						+ " is removed from your Friend list");
			} else {
				System.out.println(entitySecond.getEmail()
						+ " is not friend of you");
			}
	...
	void removeNode(Entity p) {
		...
		System.out
				.println("the person was successfully removed from the network.");
	}
