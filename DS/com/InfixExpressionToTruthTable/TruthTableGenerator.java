 ...
 * @description this class has some functions to create truth table for a expression
 */
package com.InfixExpressionToTruthTable;
...
public class TruthTableGenerator {
	/**
	 * @name truthTableGenerator()
	 * @description Method to generate truth table for given input
	 * @param input
	 *            : input expression
	 * @return
	 */
	public void truthTableGenerator(String input) {
		...
		if (Validation.isExpression(input)) {
			...
			// If input string is validated successfully then this part will be
			// executed
	...
	/**
	 * @name furnish()
	 * @description Method to furnish the input expression i.e. this method will
	 *              remove pairs of '~' symbols from expression
	 * @param input
	 *            : input expression
	 * @return input(furnished input expression)
	 */
	...
	/**
	 * @name evalPostfix()
	 * @description Method to evaluate postfix input expression
	 * @param inputPostfix
	 *            : postfix expression
	 * @return
	 */
	private void evalPostfix(String inputPostfix) {
		...
		// letterSet will contain unique characters from the input string
		Set<Character> letterSet = countletters(inputPostfix);
		char res = ' ';

		// generating binary combinations from the size of letterSet and stores
		// them to character array
			...
			for (j = 0; j < inputUpdate.length(); j++) {
				...
				if (ch == ' ') {
					...
					if (Character.isLetter(ch)) {
						...
						/*
						 * if character is not operator then pop one character
						 * from stack and perform not operation on that and
						...
						if (ch == '~') {
							...
							case '|':
								list.push(operationPerform(c1, c2, '|'));
								break;
							// This Code will be executed if the order of
							// operators is invalid
							default:
	 ...
	 * @description method to perform binary operations
	 * @param c1
	 *            : first character
	 * @param c2
	 *            : second character
	...
	private Character operationPerform(char c1, char c2, char op) {
		...
		if (op == '&') {
			// If both characters are 'F' then retrun false, otherwise return
			// true
			if (c1 == 'F' || c2 == 'F') {
				return 'F';
			} else
		...
		// If operator is '|' then this code will be executed
			...
			// If both characters are 'T' then retrun true, otherwise return
			// false
			if (c1 == 'T' || c2 == 'T') {
				return 'T';
			} else
	...
	/**
	 * @name operationPerform()
	 * @description Method to perform boolean not operation
	 * @param c1
	 *            : input character
	 * @param op
	 *            : not operator
	...
	/**
	 * @name postfixConvertor()
	 * @description MEthod to convert infix to postfix
	 * @param input
	 *            : input string in infix
	...
	private String postfixConvertor(String input) {
		...
		for (int i = 0; i < inputLength; i++) {
			...
			if (Character.isLetter(temp)) {
				// If the character is alphabet, then add the character to
			...
			// if the character is an operator
				...
				if (stackObj.size() == 0) {
					...
					// executed
						...
						// Popping characters from top of the stack until the
						// character is not equal to '|' or '('
					...
					// executed
						...
						// Popping characters from top of the stack until the
						// character is not ewual '(' or stack is empty
